...
GPU 将光栅化后的像素进行分组, 将每个组传递给不同的一个或多个 SM, SM 内所有虚拟处理器运行着相同的片段着色器, 每个虚拟处理器负责处理一个像素.
...
ByteBuffer 提供了两种方式来对堆外内存进行写入和读取, 分别是 `顺序写入/读取` 和 `随机读取/写入`. 下个小节将分别详细介绍两种读写方式, 两种方法各有优劣.
...
因为顺序写入/读取依赖于 position 进行下一次写入/读取的定位, 所以写入完成后需要使用 flip 方法重置 position 为 0 并设置最大有效数据大小, 确保读取从头开始而非从写入完后的 position 开始.
...
那么, 假设我们不想要依赖 position 定位的 "顺序写入/读取", 而是需要在指定偏移进行写入或读取, 我们该怎么办呢? ByteBuffer 提供了对应写入与读取方法的重载, 即 "随机写入/读取".
...
上述代码描述了如何使用 "随机写入/读取" 向 ByteBuffer 所包装的内存中的指定位置写入或读取数据. 此时我们使用传入的 index 参数而非 position 进行定位, position 不会在写入或读取后自增.
