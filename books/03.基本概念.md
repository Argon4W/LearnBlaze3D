...
> [!NOTE]
> 从用户视角来看 SIMT 确实将计算单元包装扩展成了 "虚拟逻辑处理器", 但这里的用词是 "分配" 和 "虚拟",
...
> [!TIP]
> 如果你没有接触过 C/C++ 等原生语言开发, 没了解过指针与地址的概念, 那我们可以这么理解:
...
- `position`: 代表了 "下一次写入或读取的地址偏移 (Offset)", 初始为0, 会随着顺序写入或读取不断自增长.
...
- `ByteBuffer.position(int newPosition)`: 设置当前的 position 为 newPosition.
- `ByteBuffer.position()`: 获取当前的 position.
...
> 当你需要读取时, 你要把视线挪回笔记本的开头 (flip) 从头开始阅读, 否则继续往后看到的是没有写上内容的空白页.
