...
图元 (Primitive) 是由顶点连线相连组成的基本几何图形. 最基本也是最常用的图元是三角形 (Triangle). 我们渲染的物体通常是网格模型 (Mesh), 即由顶点组成图元而组成的多面体.
...
此时就需要通过 [标准化设备坐标系](https://en.wikipedia.org/wiki/Clip_coordinates) (以下简称NDC) 来确定给定坐标点在窗口中的位置.
...
下图来自 [Wikipedia](https://commons.wikimedia.org/wiki/File:Cpu-gpu.svg), 原作者 Nvidia, 采用 [Creative Commons](https://en.wikipedia.org/wiki/Creative_Commons) [Attribution 3.0 Unported](https://creativecommons.org/licenses/by/3.0/deed.en)
许可协议进行授权.
...
void foo() {
    ...
    */
    ...
    var a = MemoryUtil.memGetInt(0); // 从内存地址 + 0 byte 的位置读取一个 int 的值, 读到 9876.
    var b = MemoryUtil.memGetByte(5); // 从内存地址 + 5 byte 的位置读取一个 byte 的值, 读到 44.
    var c = MemoryUtil.memGetFloat(8); // 从内存地址 + 8 byte 的位置读取一个 float 的值, 读到 15.0f.
