...
public static void setup() {
	...
	// 申请一片 GPU 内存.
    buffer = device.createBuffer(() -> "A piece of GPU memory", GpuBuffer.USAGE_VERTEX | GpuBuffer.USAGE_COPY_DST, 36);
}
```
...
|   参数    |         类型         |               含义                |
|:-------:|:------------------:|:-------------------------------:|
| `label` | `Supplier<String>` | 标签, 代表了这片 GPU 内存的名字, 可为 `null`. |
| `usage` |       `int`        |       用途, 代表了这片内存能被用来做什么.       |
| `size`  |       `long`       |  大小, 我们要申请的 GPU 内存的大小, 以字节为单位.  |

通过 createBuffer 获得的 GpuBuffer 封装了大部分我们能直接对 GPU 内存进行的操作, 其提供的方法如下:
...
但除了 `close()` 外, 在上述申请 GPU 内存的代码和 GpuBuffer 所封装的方法中, 仍然有两个未知的概念: 
1. "GPU 内存切片" (GpuBufferSlice) 是什么? 它的作用是什么?
2. "GPU 内存的用途" (usage) 是什么? 他为什么是一个整型? 又怎么计算出对应用途的整型数呢?
...
`GpuBuffer.USAGE_VERTEX | GpuBuffer.USAGE_COPY_DST` 的方式组合出 GpuBuffer 所能识别的 GPU 内存用途.
...
flowchart TB
    subgraph 传统方式 
        ...
        CMD11<-->|校验, 陷入内核态并返回| DRIVER11
        CMD12<-->|校验, 陷入内核态并返回| DRIVER12
        CMD13<-->|校验, 陷入内核态并返回| DRIVER13
    end
...
所提供的 `close()` 方法表明它**需要在完成写入或读取数据后手动停止映射**, 或使用 `try-with-resources`.

> [!IMPORTANT]
> **一定要在完成写入或读取后停止映射!** 否则你可能会遇到包括但不限于崩溃, 渲染错误, 数据没及时更新到 GPU 等各种各样的问题,,
...
> Blaze3D 才要求所有被映射的内存写入完后都要象征性地停止映射映射内存. 至于真的有没有停止映射, 则由后端自行决定. 同时, 手动释放也是在告诉 Blaze3D,
> 那些积压在 CPU 里还没来得及通过 PCI-E 发车同步到 GPU 的内存现在可以发车了, 确保所有数据都被同步到 GPU.
...
则需要将 `USAGE_MAP_WRITE` 加入到申请 GPU 内存时的 usage 标志位中:
...
为了解决这个问题, GPU 内置了一个专门的设备 "复制引擎" (Copy Engine). 它就像一台专门用于搬运的起重机, 负责从 GPU 本身或 CPU 大规模拉取数据并复制到指定 GPU 内存区域. **专门用于大批量的数据传输**.
...
|      参数       |        类型        |                含义                 |
|:-------------:|:----------------:|:---------------------------------:|
| `destination` | `GpuBufferSlice` | 代表了我们传入的堆外内存数据需要被复制到 GPU 内存上的哪个区域 |
| `ByteBuffer`  |      `data`      |     代表了需要被复制到 GPU 内存上的堆外内存数据      |

从名字不难看出, 它将数据 **复制** 进了目标的 GPU 内存. 因此我们的目标内存需要支持 "数据被复制到这片内存里" 这个用途, 即 `GpuBuffer.USAGE_COPY_DST`.
我们需要将它加入到申请 GPU 内存时的 usage 位标志中:
...
public static void setup() {
    ...
    buffer = device.createBuffer(
            () -> "Name of the memory", 
            GpuBuffer.USAGE_COPY_DST, // 使用 USAGE_COPY_DST 告诉 GPU 设备我们需要这片内存来接受被复制过来的数据.
            1024
    );
...
> 请不要认为它和 glBufferSubData一样. 在 Vulkan 后端中它是 **不保证正在读写的数据不被复制写入干扰的**. 
